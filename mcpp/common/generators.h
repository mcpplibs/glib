...
// picked up.
struct enum_def {
    std::string name;                  // GUnicodeType
    std::string prefix;                // G_UNICODE
...
{
    if (e.values.empty()) return;
    // Longest common prefix of the enumerator names, cut at the last '_'.
    ...
    e.prefix = lcp.substr(0, cut);

    // The short name is the type name with the prefix's words removed, upper
    // cased and underscore separated: GUnicodeBreakType + G_UNICODE_BREAK →
    // TYPE.
    std::string words;
    for (std::size_t i = 1; i < e.name.size(); ++i) {   // skip the leading 'G'
        if (std::isupper((unsigned char)e.name[i]) && !words.empty()) words += '_';
        words += char(std::toupper((unsigned char)e.name[i]));
    }
    const std::string full = "G_" + words;
    e.shortname = (full.size() > e.prefix.size() + 1
                   && full.compare(0, e.prefix.size(), e.prefix) == 0)
                      ? full.substr(e.prefix.size() + 1)
                      : words;
}
    ...
    while (std::getline(in, line)) {
        if (!inside) {
            if (line.find("typedef enum") != std::string::npos) {
                inside = true;
                cur = enum_def{};
                // `/*< flags >*/` on the same line marks a flags type, which
                // mkenums renders with g_flags_register_static instead.
                cur.flags = line.find("flags") != std::string::npos;
            }
            continue;
        }
        ...
        if (e > s) cur.values.push_back(line.substr(s, e - s));
    }
    return out;
}
...
void gen_enumtypes()
{
    ...
    struct { const char *tpl; const char *out; } jobs[] = {
        {"glib-enumtypes.h.template", "glib-enumtypes.h"},
        {"glib-enumtypes.c.template", "glib-enumtypes.c"},
    };
    fs::create_directories(g_inc / "gobject");
    for (const auto &j : jobs) {
        const fs::path tplPath = g_up / "gobject" / j.tpl;
        ...
        o << subst(section(tpl, "file-production"), "@filename@", "gunicode.h");
        for (const auto &e : enums) {
            std::string vh = section(tpl, "value-header");
            vh = subst(vh, "@enum_name@", [&] {
                // GUnicodeType → g_unicode_type
                std::string n;
                for (std::size_t i = 0; i < e.name.size(); ++i) {
                    if (std::isupper((unsigned char)e.name[i]) && i) n += '_';
                    n += char(std::tolower((unsigned char)e.name[i]));
                }
                return n;
            }());
            vh = subst(vh, "@EnumName@", e.name);
            vh = subst(vh, "@ENUMPREFIX@", e.prefix);
            vh = subst(vh, "@ENUMSHORT@", e.shortname);
            vh = subst(vh, "@type@", e.flags ? "flags" : "enum");
            vh = subst(vh, "@Type@", e.flags ? "Flags" : "Enum");
            // The .c template's value-production/value-tail sections list every
            // enumerator; the .h template has none, so both are handled the
            // same way here.
            std::string body;
            for (const auto &v : e.values) {
                ...
                vp = subst(vp, "@valuenick@", [&] {
                    // mkenums' default nick: the enumerator minus the prefix,
                    // lower cased with underscores turned into hyphens.
                    std::string n = v.size() > e.prefix.size() + 1
                                        ? v.substr(e.prefix.size() + 1) : v;
            ...
            vt = subst(vt, "@Type@", e.flags ? "Flags" : "Enum");
            vt = subst(vt, "@ENUMNAME@", [&] {
                std::string n;
                for (std::size_t i = 0; i < e.name.size(); ++i) {
                    if (std::isupper((unsigned char)e.name[i]) && i) n += '_';
                    n += char(std::toupper((unsigned char)e.name[i]));
                }
                return n;
            }());
            vt = subst(vt, "@ENUMSHORT@", e.shortname);
            vt = subst(vt, "@ENUMPREFIX@", e.prefix);
            o << vh << body << vt;
        }
        o << section(tpl, "file-tail");

        std::ofstream f(g_inc / "gobject" / j.out);
        f << o.str();
    }
