// gio — the fourth member, exercised rather than merely linked.
//
...
//   CHECKED-IN CODEGEN    mcpp/generated/*.c — 15,392 lines that no build step
//                         here produces. If they were stale, wrong, or from a
//                         different codegen version, they would still compile.
...
// No extern "C" — see mcpp/gobject/tests/gobject.cpp for the libc++ reason.
#include <gio/gio.h>

// The checked-in gdbus-codegen output, included by its own name. This is not
// gio public API; it is deliberately reached for, because it is the only way to
// ask the 15,392 generated lines a question they could get wrong.
#include <xdp-dbus.h>
...
// ── the checked-in gdbus-codegen output ─────────────────────────────────────
//
// `gxdp_documents_interface_info()` walks a static table the generator emitted
// and hands back a GDBusInterfaceInfo. If mcpp/generated/ were stale, empty, or
// produced from different XML, this is where it shows — and NOWHERE else,
// because the four gio sources that call into it only run inside a Flatpak
// sandbox with a portal on the bus.
void test_codegen_output()
{
    GDBusInterfaceInfo *info = gxdp_documents_interface_info();
    check(info && info->name && std::strcmp(info->name, "org.freedesktop.portal.Documents") == 0,
          "gdbus-codegen: the Documents portal interface name");

    GDBusMethodInfo *m = info ? g_dbus_interface_info_lookup_method(info, "AddNamedFull")
                              : nullptr;
    check(m != nullptr, "gdbus-codegen: AddNamedFull is in the emitted method table");
    check(m && m->in_args && m->out_args, "gdbus-codegen: the method carries its signature");

    check(gxdp_documents_get_type() != 0, "gdbus-codegen: the skeleton GType registers");
}
